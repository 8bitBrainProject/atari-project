...
for curr_file in range(len(datafiles)):
    ...
    if (len(eps) > max_eps):
        eps = eps[0:(max_eps - 1)]
        errors = errors[0:(max_eps - 1)]

    # Rolling average
    avgerrors = numpy.convolve(errors, numpy.ones(25), 'valid') / 25
    avgeps = eps[0:len(avgerrors)]
    # plt.plot(avgeps, avgerrors, label = data_label)

    # # Literal mean -- useless
    # mean = numpy.zeros(len(errors))
    # total = 0
    ...
    # Running mean
    runningmean = numpy.zeros(len(errors))
    runningmean[0] = errors[0]
    for i in range(1, len(errors)):
        runningmean[i] = runningmean[i - 1] * 0.99 + errors[i] * 0.01

    plt.plot(eps, runningmean, label = data_label)

    # Fit exponential function
    # pars, cov = curve_fit(f=exponential, xdata=eps, ydata=errors, p0=[0, 0], bounds=(-numpy.inf, numpy.inf))
    # print(pars)
