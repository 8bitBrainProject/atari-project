        ...
        try:
            ...
            while (curr_line):
                ...
                if (filename.startswith('ppo')):
                    ...
                    run_error_lists[curr_episode][curr_run] = 21.0 - float(data_list[5])
                else:
                    run_episodes[curr_episode] = int(data_list[0])
                    run_error_lists[curr_episode][curr_run] = 21.0 - float(data_list[1])
                curr_line = reader.readline()
                curr_episode += 1
        except:
...
# plt.title('Random Error over 30 Runs (mean, min, max, std)')
plt.xlabel('Episodes')
plt.ylabel('Error')
...
# plt.savefig('plots/Random Error over 30 Runs.png', dpi = 600)
