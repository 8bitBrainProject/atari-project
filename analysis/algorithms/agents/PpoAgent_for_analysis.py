...
def run_ppo(policy, run_no):
    ...
    env = make_atari('PongNoFrameskip-v4')  # DGB
    env = wrap_deepmind(env) # DGB
    env.reset()
    ...
    reward_log = open('ppo pv4w 1e-4 no' + str(run_no) + '.txt', 'w')
    ...
    for it in range(50):
        ...
        # update policy
        for _ in range(5):
            n_batch = 5000  # DGB 24576 reduced to 5000 since action history is smaller now (reduced 34075 -> 7502)
